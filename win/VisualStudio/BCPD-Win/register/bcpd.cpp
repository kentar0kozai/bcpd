...
static void print_status(int lp, double Np, double sigma, double diff, double conv, int opt, int J, int local) {
    const char *acc = (local && (opt & PW_OPT_LOCAL)) ? "KDtree " : (J ? "Nystrom" : "None   ");
    if (opt & PW_OPT_QUIET)
        return;
    if (opt & PW_OPT_HISTO) {
        if (lp)
    ...
    for (lp = 0; lp < nlp; lp++) {
        ...
        *Np = 0;
        ...
        if (opt & PW_OPT_PATHX)
            for (m = 0; m < M; m++)
                for (d = 0; d < D; d++)
                    sx[d + D * m + D * M * lp] = x[d + D * m];
        if (opt & PW_OPT_PATHY)
            for (m = 0; m < M; m++)
                for (d = 0; d < D; d++)
                    sy[d + D * m + D * M * lp] = y[d + D * m];
        /*---------------------------------------------------------------o
        |   update: u, v                                                 |
        o---------------------------------------------------------------*/
        cc = lmd * SQ(*r / (*s));
...
#pragma omp parallel for private(j)
            for (i = 0; i < M; i++)
                for (j = i; j < M; j++)
                    G1[i + M * j] = G1[j + M * i] = G[i + M * j] + (i == j ? cc / w[i] : 0);
...
#pragma omp parallel for private(d) private(i)
            for (m = 0; m < M; m++)
                for (d = 0; d < D; d++) {
                    u[d + D * m] = Y[d + D * m];
                    for (i = 0; i < M; i++)
                        u[d + D * m] += G[m + M * i] * E[i + M * d];
                }
            if (db) {
                tr = 0;
                for (i = 0; i < M; i++)
                    for (j = i; j < M; j++)
                        G2[i + M * j] = G2[j + M * i] = G[i + M * j];
        ...
        for (i = 0; i < D; i++)
            for (j = 0; j < D; j++)
                c1 += R[i + D * j] * Sxu[i + D * j];
        for (d = 0; d < D; d++)
            for (m = 0; m < M; m++)
                c2 += w[m] * (SQ(u[d + D * m] - ub[d]));
        *s = c1 / c2;
        /* t */
        for (d = 0; d < D; d++) {
            val = 0;
